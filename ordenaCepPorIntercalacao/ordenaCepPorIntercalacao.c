...
int main()
{
    ...
    {
        if (i == qtdBlocos - 1)            //Se estiver no ultimo loop.
            qtdRegPorBloco = qtdRegistros; // Irá criar o último bloco com a quantidade de registros que falta.
    ...
    for (int etapa = log2(qtdBlocos); etapa > 0; etapa--)
    {
        ...
        for (int i = 0; i < qtdBlocos; i++)
        {
            ...
            // Se estiver na última intercalação o nome do arquivo será diferente.
            (etapa == 1) ? sprintf(arqSaida, "cep_ordenado_intercalacao.dat") : sprintf(arqSaida, "cep_bloco_%d.dat", ultimoArquivo++);
